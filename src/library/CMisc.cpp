...
void CMisc::formatDate2(const char *sourceDate, char *destDate) {
	...
	*destDate = 0; // EOS
	...
	//	memcpy(destDate + 4, sourceDate + 5, 2);
	//	memcpy(destDate + 6, sourceDate + 8, 2);
	//
	//	memcpy(destDate + 8, sourceDate + 11, 2);
	//	memcpy(destDate + 10, sourceDate + 14, 2);
	//	memcpy(destDate + 12, sourceDate + 17, 2);
	//
	//	memcpy(destDate + 14, ".0", 2);

	//printf ("\nformatDate2: dateIn:=%s, dateOut=%s", sourceDate, dest);
	//printf (" ");
...
bool CMisc::isDate(char *stringPtr, int len) {
	...
	if (s.StartsWith("sec.") || s.EndsWith("sec.")) // secolo
		...
		return true;
	else if (s.StartsWith("m.") || s.EndsWith(" m.")) // morto
		return true;
	//	else if (s.StartsWith("n.") || s.EndsWith(" n.")) // nato toppa per "N.J. : Prentice-Hall" 20/11/2010
	else if (s.StartsWith("n. ") || s.EndsWith(" n.")) // nato
		return true;
	...
	&& (isdigit(*(stringPtr + 2)) || *(stringPtr + 2) == '.') && (isdigit(
			*(stringPtr + 3)) || *(stringPtr + 3) == '.'))
		return true;

	if (isdigit(*stringPtr) // eg. <nn/nn>
			&& isdigit(*(stringPtr + 1)) && *(stringPtr + 2) == '/' && isdigit(
			*(stringPtr + 3)) && isdigit(*(stringPtr + 4)))
		return true;

	if (isdigit(*stringPtr) // eg. <18/t-1825p>
			&& isdigit(*(stringPtr + 1)) && *(stringPtr + 2) == '/'

	&& (*(stringPtr + 3) == 't' || *(stringPtr + 3) == 'i' || *(stringPtr + 3)
			== 'f' || *(stringPtr + 3) == 'm' || *(stringPtr + 3) == 'p'
			|| *(stringPtr + 3) == 's'

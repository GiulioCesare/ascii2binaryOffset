...
OrsChar *CString::SubstringData(OrsLong From, OrsLong Length)
   	{
//	OrsInt Length = To - From +1 ;

	// In range?
	if	(From < 0 || From >= UsedBytes || Length < 0 || Length >= UsedBytes)
		{
	    SignalAnError(__FILE__, __LINE__, "SubstringData out of range, from=%d, length=%d, strlen=%d string=%s", From, Length, UsedBytes-1, this->Buffer);
		return OrsNULL;
		}

	// Buffer for substring big enough?
	if	(BufferSubStringSize <= Length)
		{
		// Need bigger buffer
		if	(BufferSubString)
			free (BufferSubString);

		BufferSubString = (OrsChar *)malloc (Length +1); // Con EOS
		if	(!BufferSubString)
			{
			SignalAnError(__FILE__, __LINE__, "CString malloc failed for Length %d", Length);
			return OrsFALSE;
			}
		BufferSubStringSize = Length + 1;
		}

	// Copy substring from Buffer to BufferSubString buffer
	BufferSubStringLength = Length;
...
OrsBool CString::AppendString(OrsChar *aString, OrsLong Size)
	{
	// Esiste un buffer
	if	(!Buffer)
		{
	...
	// Abbiamo ancora spazio nel buffer?
	else if	(!UnusedRightBytes || UnusedRightBytes < Size)
		{
		long SizeToAllocate = ResizeBy;
		while (SizeToAllocate < Size)
			SizeToAllocate += ResizeBy;
	...
	memcpy(BufTailPtr, aString, Size);
